...
TS=$(date +%Y%m%d-%H%M%S)
RUN_ID="${TS}_factor_eval_theme=${THEME}_h=${HORIZON}_q=${QUANTILES}_reb=${REBALANCE}_ic=${IC_METHOD}_${START}-${END}"
...
docker exec ${CONTAINER} bash -lc "python /tmp/evaluate_factors.py --start ${START} --end ${END} --theme '${THEME}' --factor-parquet /tmp/factor_values.parquet --horizon ${HORIZON} --quantiles ${QUANTILES} --rebalance ${REBALANCE} --ic-method ${IC_METHOD} --outdir /tmp/output" | tee "${OUTDIR_HOST}/console.txt"
