...
TS=$(date +%Y%m%d-%H%M%S)
RUN_ID="${TS}_factor_bt_theme=${THEME}_fac=${FACTOR}_reb=${REBALANCE}_dir=${DIRECTION}_top=${TOPK}_q=${QUANTILE:-none}_cost=${COST_BPS}_${START}-${END}"
...
docker exec ${CONTAINER} bash -lc "python /tmp/backtest_factor_portfolio.py --start ${START} --end ${END} --theme '${THEME}' --factor-parquet /tmp/factor_zscore.parquet --factor '${FACTOR}' --rebalance ${REBALANCE} --direction ${DIRECTION} --topk ${TOPK} ${QUANTILE:+--quantile ${QUANTILE}} --cost-bps ${COST_BPS} --outdir /tmp/output" | tee "${OUTDIR_HOST}/console.txt"
